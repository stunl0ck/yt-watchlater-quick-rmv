...
(function () {
    ...
    function addQuickRemoveButtons() {
        const videoItems = document.querySelectorAll('ytd-playlist-video-renderer:not([data-quick-remove-added])');
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        mutations.forEach((mutation) => {
            // Only check if actual playlist video elements were added
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1 && (
                    node.tagName === 'YTD-PLAYLIST-VIDEO-RENDERER' ||
                    node.querySelector && node.querySelector('ytd-playlist-video-renderer')
                )) {
                    shouldCheck = true;
                }
            });
    ...
    setInterval(() => {
        if (location.href !== currentUrl) {
            ...
            if (currentUrl.includes('playlist?list=WL')) {
